  ...
  useEffect(() => {
    const fetchActivities = async () => {
      try {
        ...
        setActivities(list.slice(0, 8)); // Top 8 activities
      } catch (e) {
      ...
      >
        ...
        <div id="right-panel-viewport" className="flex-1 overflow-y-auto scrollbar-ephemeral p-5 space-y-5">
          {activeRightWidget === 'ai-suggestions' && (
            <div id="widget-ai-suggestions" className="space-y-4 animate-fade-in text-left">
              ...
              {activeView === 'dashboard' ? (
                <div className="space-y-4" id="ai-dashboard-prompts-container">
                  <span className="text-[10px] font-bold font-mono tracking-wider text-cinema-amber-500 uppercase block border-b border-border pb-1">
                    Intelligent Director Prompts
                  </span>
                  {[
                    {
                    ...
                    <div
                      key={idx}
                      className="p-3 rounded-xl border border-border bg-card/60 space-y-2 relative group"
          ...
          {activeRightWidget === 'versions' && (
            <div id="widget-versions" className="space-y-4 animate-fade-in text-left">
              {activeView === 'dashboard' ? (
                <div className="space-y-4">
                  <h4 className="font-display text-sm font-semibold text-foreground flex items-center gap-2">
                    <History className="w-4 h-4 text-muted-foreground" /> Chronological Activity Log
                  </h4>
                  <p className="text-xs text-muted-foreground leading-relaxed">
                  ...
                  {activities.length > 0 ? (
                    <div className="relative border-l border-border pl-4 space-y-4 ml-1.5 pt-2" id="inspector-activity-timeline-list">
                      {activities.map((act, idx) => (
                        <div key={act.id || idx} className="relative" id={`inspector-activity-timeline-item-${idx}`}>
                          {/* Bullet Circle */}
                          <span className={`absolute -left-[21.5px] top-1 w-2 w-2 rounded-full border border-card ${act.iconColor || 'bg-cinema-amber-500'}`} />
                          <div className="space-y-0.5 text-left" id={`inspector-activity-timeline-item-${idx}-content`}>
                    ...
                    <p className="text-xs text-muted-foreground italic">No activities registered yet.</p>
                  )}
                </div>
              ) : (
                <>
