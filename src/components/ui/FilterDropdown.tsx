      ...
      {isOpen && (
        <>
          ...
          <div
            ...
            } mt-1.5 bg-card border border-border rounded-xl shadow-lg p-1.5 z-50 text-left text-xs text-foreground flex flex-col gap-1 overflow-y-auto ${maxHeight} ${dropdownClassName}`}
            id={`${id}-dropdown`}
          >
            {options.map((option) => (
              <button
                ...
                className={`w-full px-3 py-2 text-left font-semibold rounded-lg transition-colors flex items-center justify-between cursor-pointer ${
                  value === option.value
